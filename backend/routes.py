...
def my_trips():
    ...
    user_trips = (
        db_session.query(Trip).join(Profile).filter(Profile.user_id == user_id).all()
    )
    ...
    for trip in user_trips:
        ...
        if creator_profile:
            creator = (
                ...
                .filter(User.id == creator_profile.user_id)
                .first()
            )
        ...
        trips.append(
            {"trip_id": trip.id, "trip_name": trip.name, "creator_name": creator_name}
        )
