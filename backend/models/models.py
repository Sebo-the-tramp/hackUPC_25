...
class Trip(Base):
    ...
    # Define relationship with Profile model (one-to-many)
    profiles = relationship("Profile", back_populates="trip", cascade="all, delete-orphan")
...
class Profile(Base):
    ...
    def __repr__(self):
        return f"<Profile(id={self.id}, user_id={self.user_id}, trip_id={self.trip_id})>"
...
class Message(Base):
    ...
    profile_id = Column(Integer, ForeignKey('profiles.id'), nullable=True)  # Nullable for AI messages
