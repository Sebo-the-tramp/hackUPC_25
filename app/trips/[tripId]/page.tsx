...
const TripPage: React.FC<TripPageProps> = ({ params }) => {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
        <div className="w-full max-w-md p-6 bg-white rounded-xl shadow-lg text-center">
          <h1 className="text-2xl font-bold text-red-600 mb-4">Error Loading Trip</h1>
  ...
  const formattedMembers = tripData.trip.users.map((user) => ({
    id: user.id.toString(),
    name: user.name,
    profile_id: 0, // We don't have this info from the API
    homeAirport: null, // We don't have this info from the API yet
  }));

  // For now, assume the first user is the creator
  const creator = {
    id: formattedMembers[0]?.id || "1",
    name: formattedMembers[0]?.name || "Trip Creator",
    profile_id: 0,
    homeAirport: null,
  };

  // All users are members since the creator is included in the users array
  const members = formattedMembers;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white p-4 rounded-lg shadow-md mb-4">
          <div className="flex items-center justify-between mb-3">
            <h1 className="text-2xl font-bold text-indigo-700">{tripData.trip.name}</h1>
            <button
              onClick={() => router.push("/")}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-[calc(100vh-16rem)]">
          ...
          <div className="bg-white rounded-lg shadow-md h-full">
            <ChatInterface
              tripId={tripId}
              initialMessages={(tripData.trip.messages || []).map((msg) => ({
                id: msg.id.toString(),
                sender: msg.user?.id == undefined ? "llm" : "user", // Assuming user_id 0 is for AI messages
                text: msg.content,
                timestamp: new Date(), // The backend doesn't currently return timestamps
                senderName: msg.user?.name || "AI",
              }))}
            />
          </div>
          ...
          <div className="flex flex-col gap-4 h-full">
            ...
            <div className="bg-indigo-50 p-4 rounded-lg shadow-md flex-1 min-h-[40%]">
              <div className="flex justify-between items-center mb-3">
                ...
                <span className="text-sm text-indigo-600 font-medium">{allMembers.length} members</span>
              </div>
            ...
            <div className="z-0 bg-white rounded-lg shadow-md flex-1 min-h-[45%]">
              <TripMap members={allMembers.filter((member) => !!member.homeAirport)} />
            </div>
          </div>
        </div>
      </div>
